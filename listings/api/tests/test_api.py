...
class ListingAPITests(TestCase):
    ...
    def test_filter_by_bedrooms_and_bathrooms_comma_separated(self):
        """Test filtering listings by comma-separated list of bedrooms and bathrooms"""
        url = reverse("listing-list")

        # Test with bedrooms=2,3 and bathrooms=1,2.0
        response = self.client.get(url, {"bedrooms": "2,3", "bathrooms": "1,2.0"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # ORM equivalent: bedrooms in [2, 3] and bathrooms in [1, 2.0]
        expected_qs = Listing.objects.filter(bedrooms__in=[2, 3], bathrooms__in=[1.0, 2.0])
        self.assertEqual(response.data["count"], expected_qs.count())
        for item in response.data["results"]:
            self.assertIn(item["bedrooms"], [2, 3])
            self.assertIn(float(item["bathrooms"]), [1.0, 2.0])

        # Test that old single-value filtering still works
        response = self.client.get(url, {"bedrooms": 3, "bathrooms": 2})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        expected_qs = Listing.objects.filter(bedrooms=3, bathrooms=2.0)
        self.assertEqual(response.data["count"], expected_qs.count())
        for item in response.data["results"]:
            self.assertEqual(item["bedrooms"], 3)
            self.assertEqual(float(item["bathrooms"]), 2.0)
